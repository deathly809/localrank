		     ...
		     optimization if not NULL. Array must be of size totdoc. */
{
  ...
  learn_parm->eps               = 0.0;      /* No margin, unless explicitly handcoded in the right-hand side in the training set.  */
  ...
  if(alpha) {
    ...
    (void)calculate_svm_model(docs,label,unlabeled,lin,alpha,a,c,learn_parm,index2dnum,index2dnum,model);
    for(i=0;i<totdoc;i++) {    /* copy initial alphas */
      a[i]=alpha[i];
    }
  ...
  if(learn_parm->sharedslack) {
    iterations=optimize_to_convergence_sharedslack(docs,label,totdoc,totwords,learn_parm,kernel_parm,kernel_cache,&shrink_state,model,a,lin,c,&timing_profile,&maxdiff);
  }else {
    iterations=optimize_to_convergence(docs,label,totdoc,totwords,learn_parm,kernel_parm,kernel_cache,&shrink_state,model,inconsistent,unlabeled,a,lin,c,&timing_profile,&maxdiff,(long)-1,(long)1);
  }
