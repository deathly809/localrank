		      ...
		      STRUCTMODEL *sm, int alg_type)
{
  ...
  int         opti_round, *opti, use_shrinking;
  ...
  svm_learn_optimization(cset.lhs,cset.rhs,cset.m,sizePsi+n,lparm,kparm,kcache,svmModel,alpha);
  if(kcache)
  ...
  do { /* iteratively increase precision */
	...
	for(i=0; i<n; i++) { /*** example loop ***/
	  ...
	  if((!use_shrinking) || (opti[i] != opti_round)) { /* if the example is not shrunk away, then see if it is necessary to add a new constraint */
	    rt2=get_runtime();
	    argmax_count++;
			    ...
			    STRUCTMODEL *sm, int alg_type)
{
  ...
  do { /* iteratively find and add constraints to working set */
      ...
      if(alg_type == ONESLACK_DUAL_CACHE_ALG) {
        ...
        if(struct_verbosity>=2) { /* Compute violation of constraints in cache for current w */
          rt2=get_runtime();
        }  
        update_constraint_cache_for_model(ccache, svmModel);
        if(struct_verbosity>=2) {
          rt_cacheupdate+=MAX(get_runtime()-rt2,0);
        }
        
        viol  = compute_violation_of_constraint_in_cache(ccache,epsilon_est/2);  /* Is there is a sufficiently violated constraint in cache? */
        if(viol-slack > MAX(epsilon_est/10,sparm->epsilon)) {   /* There is a sufficiently violated constraint in cache, so use this constraint in this iteration. */ 
          if(struct_verbosity>=2) {
            rt2=get_runtime();
          }
          viol  = find_most_violated_joint_constraint_in_cache(ccache,epsilon_est/2,lhs_n,&lhs,&rhs);
          if(struct_verbosity>=2) {
            rt_cacheconst += MAX(get_runtime()-rt2,0);  
          } 
          cached_constraint = 1;
        } else {
